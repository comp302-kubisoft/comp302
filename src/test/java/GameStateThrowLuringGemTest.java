...
public class GameStateThrowLuringGemTest {
  ...
  void testThrowLuringGemTypicalCase() {
    ...
    assertEquals(initialGemCount - 1, gameState.getEnchantmentCount(Enchantment.Type.LURING_GEM));
    assertTrue(
        gameState.getGemX() * TILE_SIZE
            > gameState.getHero().getX()); // Gem should be to the right of hero
  }
