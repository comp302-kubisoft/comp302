...
public class Menu {
  ...
  private void loadBackgroundImage() {
    try {
      backgroundImage =
          ImageIO.read(getClass().getResourceAsStream("/maps/mainmenubackground.png"));
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  public void draw(Graphics2D g2, int screenWidth, int screenHeight) {
    ...
    for (int i = 0; i < options.length; i++) {
      ...
      if (i == selectedOption) {
        ...
        g2.drawString(
            text,
            screenWidth / 2 - g2.getFontMetrics().stringWidth(text) / 2 + 1,
            startY + i * spacing + 1);
    ...
    g2.setColor(new Color(0, 0, 0, 128));
    g2.drawString(controls, screenWidth / 2 - controlsWidth / 2 + 1, screenHeight - 40 + 1);

    // Draw controls text
    g2.setColor(TEXT_COLOR);
    g2.drawString(controls, screenWidth / 2 - controlsWidth / 2, screenHeight - 40);
  }

  /**
   * Requires: - upPressed, downPressed, enterPressed accurately reflect key press states at the
   * time of call.
   *
   * <p>Modifies: - this.selectedOption (the currently highlighted menu option).
   *
   * <p>Effects: 1. If upPressed is true and enough time has passed since last input: - Decrement
   * selectedOption (wrap around if needed) and play a cursor sound. 2. If downPressed is true and
   * enough time has passed since last input: - Increment selectedOption (wrap around if needed) and
   * play a cursor sound. 3. If enterPressed is true and enough time has passed since last input: -
   * If currently selectedOption == 0 => Return GameMode.BUILD (start game) - If selectedOption == 1
   * => Return GameMode.LOAD - If selectedOption == 2 => Return GameMode.HELP - If selectedOption ==
   * 3 => Call System.exit(0) 4. If no option is selected by enterPress or if input is within delay,
