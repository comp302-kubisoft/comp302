/**
 * Handles mouse input events for the game's build mode. This class manages object selection from
 * the build panel and object placement in the game area through mouse interactions.
 */
package ui.input;
...
public class MouseHandler extends MouseAdapter {
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    // Check if click is on the save button in play mode
    if (gamePanel.getMode() == GameMode.PLAY && renderer.isWithinSaveButton(x, y)) {
      soundManager.playSFX(6); // Play cursor sound
  ...
  private void handleGameAreaClick(int x, int y) {
    ...
    if (gameState.isWithinGameArea(gridX, gridY) && !gameState.isTileOccupied(gridX, gridY)) {
      gameState.addPlacedObject(
          renderer.getSelectedObjectIndex(), gridX * tileSize, gridY * tileSize, gridX, gridY);
    }
  }
